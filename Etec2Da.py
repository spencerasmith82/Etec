...
#To more flexibly interact with the core Etec2D algorithm, I have been doing much of the higher-level control in separate ipython notebook files.  However, this function is a good template to base further uses of Etec2D on (I'll put some additional commented-out ideas in this function).  I'll restrict this function to taking in a file name that has a particular structure (like a matlab .mat file with a particular structure), and outputting the weights vs. time list and the estimated topological entropy and error
def GetWeightsAndEntropy(filename):
    ...
    #[times,traj] = OpenTrajectoryFile(filename)
...
class triangulation2D:
        ...
        if not rbands is None:
            for i in range(0,len(rbands)):
                ...
                for j in range(0,rblen):
                    ...
                    if self.TriArea([self.pointpos[p1],self.pointpos[p2],self.pointpos[p3]]) > 0:
    ...
    def TriangulationPlotWeights(self,filename = None,Wcutoff = None):
        xpoints = [x[0] for x in self.pointpos[:len(self.pointpos)-self.extranum]]  #note that we exclude the bounding points
        ypoints = [x[1] for x in self.pointpos[:len(self.pointpos)-self.extranum]]
        triangles = [x.points for x in self.simplist if (len(set(x.points).intersection([(len(self.pointpos)-y) for y in range(1,self.extranum+1)])) == 0)]  #make sure that the list of triangles (triplets of points) do not include the excluded large triangle points
        plt.figure(figsize=(14,14))
